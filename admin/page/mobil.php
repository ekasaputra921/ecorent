...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($update) {
		...
		$sql = "INSERT INTO mobil VALUES (NULL, '$_POST[id_jenis]', '$_POST[no_mobil]', '$_POST[merk]', '$_POST[nama_mobil]', '$file_name', '$_POST[harga]', '$_POST[status]',1)";
	}
	if (!$err) {
...
if (isset($_GET['action']) AND $_GET['action'] == 'delete') {
	$connection->query("UPDATE mobil SET isActive=0 WHERE id_mobil='$_GET[key]'");
	echo alert("Berhasil!", "?page=mobil");
}
?>
<div class="row">
				...
				<form action="<?=$_SERVER['REQUEST_URI']?>" method="POST" enctype="multipart/form-data">
					<div class="form-group">
						...
						<select class="form-control" name="id_jenis">
							<option>---</option>
							<?php $query = $connection->query("SELECT * FROM jenis WHERE isActive=1"); while ($data = $query->fetch_assoc()): ?>
							<option value="<?=$data["id_jenis"]?>" <?= (!$update) ?: (($row["id_jenis"] != $data["id_jenis"]) ?: 'selected="on"') ?>><?=$data["nama"]?></option>
						<?php endwhile; ?>
					</select>
				</div>
				<div class="form-group">
					<label for="no_mobil">Plat Nomor</label>
	...
	<div class="panel panel-info">
		...
		<div class="panel-body">
			<table class="table table-condensed">
				...
				<tbody>
					<?php $no = 1; ?>
					<?php if ($query = $connection->query("SELECT * FROM mobil m JOIN jenis j USING(id_jenis) WHERE m.isActive=1")): ?>
