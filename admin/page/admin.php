...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	if ($update) {
		...
		$sql = "INSERT INTO admin VALUES (NULL, '$_POST[nama]', '$_POST[email]', '$_POST[alamat]', '$_POST[telp]', '$_POST[username]', '".md5($_POST["password"])."',1)";
	}
...
if (isset($_GET['action']) AND $_GET['action'] == 'delete') {
	$connection->query("UPDATE admin SET isActive = 0 WHERE id_admin='$_GET[key]'");
	echo alert("Berhasil!", "?page=admin");
}
?>
<div class="row">
		...
		<div class="panel panel-info">
			...
			<div class="panel-body">
				<table class="table table-condensed">
					...
					<tbody>
						<?php $no = 1; ?>
						<?php if ($query = $connection->query("SELECT * FROM admin WHERE isActive=1")): ?>
